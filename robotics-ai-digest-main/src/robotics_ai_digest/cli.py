...
def handler_ingest(args: argparse.Namespace) -> int:
    """Fetch RSS entries and persist only new articles."""
    try:
...
def handler_list(args: argparse.Namespace) -> int:
    """Print recent articles grouped by day and source."""
    session_factory = init_db(args.db)
    with session_factory() as session:
...
def handler_run(args: argparse.Namespace) -> int:
    """Run ingest then list as a convenience orchestration command."""
...
def handler_digest(args: argparse.Namespace) -> int:
    """Generate the markdown digest for one date."""
    try:
...
def handler_summarize(args: argparse.Namespace) -> int:
    """Create AI summaries for articles that do not have one yet."""
    load_dotenv()
...
def handler_summaries(args: argparse.Namespace) -> int:
    """Display persisted AI summaries for recent articles."""
    session_factory = init_db(args.db)
    with session_factory() as session:
