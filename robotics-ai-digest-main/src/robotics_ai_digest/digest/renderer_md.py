...
def _normalize_summary(summary: str | None, max_len: int = 240) -> str | None:
    """Normalize summary text to a single line with optional truncation."""
    if not summary:
        return None
...
def render_digest(date: date, articles: list[Article]) -> str:
    """Render a daily markdown digest grouped by article source."""
    ...
    for source, source_articles in grouped.items():
        ...
        for article in source_articles:
            ...
            # Prefer AI output when available, otherwise keep the RSS summary.
